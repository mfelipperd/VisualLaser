...
## 2024-05-23 - Unthrottled scroll event listeners
**Learning:** Found multiple instances of unthrottled `window.addEventListener("scroll")` calls in components like `LandingHeader.tsx`, `StickyAppointment.tsx`, and `Header.tsx`. Unthrottled scroll events can block the main thread and cause layout thrashing and jank.
**Action:** Use `requestAnimationFrame` to throttle scroll event handlers and add `{ passive: true }` to the event listener options.
