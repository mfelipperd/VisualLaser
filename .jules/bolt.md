...
## 2024-05-24 - Unthrottled scroll event listeners
**Learning:** Unthrottled scroll event listeners using `window.addEventListener('scroll', handler)` can block the main thread and cause layout thrashing/jank during rapid scrolling. The codebase had several of these in sticky headers and appointment components.
**Action:** Always wrap scroll event handlers in `window.requestAnimationFrame` and add `{ passive: true }` to the event listener options to ensure smooth scrolling performance and prevent main thread blocking.
